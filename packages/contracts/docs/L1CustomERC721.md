...
```solidity
function l2Contract() external view returns (address)
```
...
```solidity
function mint(address _to, uint256 _tokenId, bytes _data) external nonpayable
```
...
*Override the tokenURI method to return your onChain metadata. You basically can just copy the same logic from your original ERC721 contract.*
